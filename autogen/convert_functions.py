...
def get_ffi_type(ptype):
    ...
    return ffi_types.get(ptype, '&ffi_type_pointer')

unique_signatures = {}

def get_sig_id(return_type, arg_types):
    sig = (return_type, tuple(arg_types))
    if sig not in unique_signatures:
        sig_id = return_type.replace(' ', '_').replace('*', 'p') + '__' + '_'.join([a.replace(' ', '_').replace('*', 'p') for a in arg_types])
        sig_id = sig_id.replace('struct_', '').replace('enum_', '')
        sig_id = re.sub(r'[^a-zA-Z0-9_]', '_', sig_id)
        unique_signatures[sig] = sig_id
    return unique_signatures[sig]
...
def build_lua_function_wrapper_param(pid, param, i):

    # create a comma separated list of types
    types = [alter_type(t.strip()) for t in param['fp_args']]
    types = ', '.join(types)

    sig_id = get_sig_id(param['fp_return_type'], param['fp_args'])

    # build the function wrapper
    # get a function pointer
    s = '    %s (*%s)(%s) = smlua_ffi_create_closure(smlua_to_lua_function(L, %d), smlua_ffi_bridge_%s, &smlua_ffi_cif_%s);\n' % (param['fp_return_type'], pid, types, i, sig_id, sig_id)
    s += '    if (!%s) { LOG_LUA("Failed to create closure for function %s"); return 0; }\n' % (pid, param['identifier'])
...
def build_param(fid, param, i):
    ...
    if ptype in VEC_TYPES:
        ...
        return build_lua_function_wrapper_param(pid, param, i)
    elif ptype == 'LuaTable':
...
def build_ffi_post_call(rtype):
    rtype = normalize_type(rtype)
    ptype = alter_type(rtype)
    if ptype == 'void':
        return ""

    if ptype in integer_types:
        return "    if (ret) { *(%s*)ret = smlua_to_integer(L, -1); }\n" % rtype
    if ptype in number_types:
        return "    if (ret) { *(%s*)ret = smlua_to_number(L, -1); }\n" % rtype
    if ptype == 'bool':
        return "    if (ret) { *(%s*)ret = smlua_to_boolean(L, -1); }\n" % rtype

    lot = translate_type_to_lot(ptype)
    if lot == 'LOT_POINTER':
        lvt = translate_type_to_lvt(ptype)
        return "    if (ret) { *(%s*)ret = (%s)smlua_to_cpointer(L, -1, %s); }\n" % (rtype, rtype, lvt)
    if '???' not in lot and lot != 'LOT_NONE':
        return "    if (ret) { *(%s*)ret = (%s)smlua_to_cobject(L, -1, %s); }\n" % (rtype, rtype, lot)

    return "    // UNIMPLEMENTED RETURN TYPE: %s\n" % rtype
...
def process_function(fname, line, description):
    ...
    if len(params_str) == 0 or params_str == 'void':
        ...
        for param_str in split_arguments(params_str):
            param = {}
            ...
            fp_match = re.search(r"([a-zA-Z0-9_\s\*]+)\s*\(\*([a-zA-Z0-9_]+)\)\s*\((.*)\)", param_str)
            if fp_match:
                param['type'] = 'Function'
                param['identifier'] = fp_match.group(2)
                param['fp_return_type'] = fp_match.group(1).strip()
                param['fp_args'] = [a.strip() for a in split_arguments(fp_match.group(3)) if a.strip() and a.strip() != 'void']
                param['is_fp'] = True
                get_sig_id(param['fp_return_type'], param['fp_args'])
            elif param_str.endswith('*') or ' ' not in param_str:
...
def build_ffi_bridge(sig, sig_id):
    return_type, arg_types = sig
    num_results = 0 if return_type == 'void' else 1
    s = "static ffi_cif smlua_ffi_cif_%s;\n" % sig_id
    s += "static ffi_type* smlua_ffi_args_%s[%d];\n" % (sig_id, max(1, len(arg_types)))
    s += "void smlua_ffi_bridge_%s(UNUSED ffi_cif* cif, UNUSED void* ret, UNUSED void** args, void* userdata) {\n" % sig_id
    s += "    struct LuaClosure* lc = (struct LuaClosure*) userdata;\n"
    s += "    lua_State* L = gLuaState;\n"
    s += "    if (!L) { return; }\n"
    s += "    int prevTop = lua_gettop(L);\n"
    s += "    smlua_push_lua_table(L, (LuaTable)lc->luaFunc);\n"
    s += "    if (lua_type(L, -1) != LUA_TFUNCTION) { lua_settop(L, prevTop); return; }\n"

    for i, arg_type in enumerate(arg_types):
        arg_c_type = normalize_type(arg_type)
        arg_val = "*(%s*)args[%d]" % (arg_c_type, i)
        s += build_return_value(arg_val, arg_c_type)

    s += "    if (0 == smlua_pcall(L, %d, %d, 0)) {\n" % (len(arg_types), num_results)
    if num_results > 0:
        s += "    " + build_ffi_post_call(return_type)
    s += "    }\n"
    s += "    lua_settop(L, prevTop);\n"
    s += "}\n\n"
    return s

def build_ffi_bridges():
    s = gen_comment_header("ffi bridges")
    for sig, sig_id in unique_signatures.items():
        s += build_ffi_bridge(sig, sig_id)
    return s

def build_ffi_cif_init():
    s = ""
    for sig, sig_id in unique_signatures.items():
        return_type, arg_types = sig

        for i, arg_type in enumerate(arg_types):
            s += "    smlua_ffi_args_%s[%d] = %s;\n" % (sig_id, i, get_ffi_type(arg_type))

        s += "    if (ffi_prep_cif(&smlua_ffi_cif_%s, FFI_DEFAULT_ABI, %d, %s, smlua_ffi_args_%s) != FFI_OK) {\n" % (sig_id, len(arg_types), get_ffi_type(return_type), sig_id)
        s += "        LOG_ERROR(\"Failed to prepare ffi_cif for %s\");\n" % sig_id
        s += "    }\n"
    return s
...
def main():
    ...
    built_ffi_bridges = build_ffi_bridges()
    built_ffi_cif_init = build_ffi_cif_init()
    ...
    gen = template                                \
        .replace("$[FFI_BRIDGES]", built_ffi_bridges) \
        .replace("$[FFI_CIF_INIT]", built_ffi_cif_init) \
