...
void djui_hud_set_render_behind_hud(bool enable) {
    LOG_LUA_WARNING_ONCE("djui_hud_set_render_behind_hud() is deprecated! Please use HOOK_ON_HUD_RENDER_BEHIND instead.");
    if (!gLuaActiveMod) { return; }
    gLuaActiveMod->renderBehindHud = enable;
}

f32 audio_stream_get_tempo(UNUSED struct ModAudio* audio) {
    LOG_LUA_WARNING_ONCE("audio_stream_get_tempo() is deprecated! There may be a replacement for this function in the future.");
    return 1;
}

void audio_stream_set_tempo(UNUSED struct ModAudio* audio, UNUSED f32 tempo) {
    LOG_LUA_WARNING_ONCE("audio_stream_set_tempo() is deprecated! There may be a replacement for this function in the future.");
}

void audio_stream_set_speed(UNUSED struct ModAudio* audio, UNUSED f32 initial_freq, UNUSED f32 speed, UNUSED bool pitch) {
    LOG_LUA_WARNING_ONCE("audio_stream_set_speed() is deprecated! There may be a replacement for this function in the future.");
}

void network_player_color_to_palette(struct NetworkPlayer *np, enum PlayerPart part, Color color) {
    LOG_LUA_WARNING_ONCE("network_player_color_to_palette() is deprecated! Use network_player_set_override_palette_color() instead.");
...
void network_player_palette_to_color(struct NetworkPlayer *np, enum PlayerPart part, VEC_OUT Color out) {
    LOG_LUA_WARNING_ONCE("network_player_palette_to_color() is deprecated! Use network_player_get_palette_color() or network_player_get_override_palette_color() instead.");
