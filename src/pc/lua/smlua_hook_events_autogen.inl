...
bool smlua_call_event_hooks_HOOK_ON_KEY_PRESSED(s64 key) {
    lua_State *L = gLuaState;
    if (L == NULL) { return false; }
    bool hookResult = false;

    struct LuaHookedEvent *hook = &sHookedEvents[HOOK_ON_KEY_PRESSED];
    for (int i = 0; i < hook->count; i++) {
        s32 prevTop = lua_gettop(L);
        ...
        // call the callback
        if (0 != smlua_call_hook(L, 1, 0, 0, hook->mod[i], hook->modFile[i])) {
            LOG_LUA("Failed to call the callback for hook %s", sLuaHookedEventTypeName[HOOK_ON_KEY_PRESSED]);
            continue;
        }
        hookResult = true;
...
bool smlua_call_event_hooks_HOOK_ON_KEY_RELEASED(s64 key) {
    lua_State *L = gLuaState;
    if (L == NULL) { return false; }
    bool hookResult = false;

    struct LuaHookedEvent *hook = &sHookedEvents[HOOK_ON_KEY_RELEASED];
    for (int i = 0; i < hook->count; i++) {
        s32 prevTop = lua_gettop(L);
        ...
        // call the callback
        if (0 != smlua_call_hook(L, 1, 0, 0, hook->mod[i], hook->modFile[i])) {
            LOG_LUA("Failed to call the callback for hook %s", sLuaHookedEventTypeName[HOOK_ON_KEY_RELEASED]);
            continue;
        }
        hookResult = true;
