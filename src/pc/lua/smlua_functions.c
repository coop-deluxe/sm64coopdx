...
int smlua_func_init_mario_after_warp(lua_State *L) {
    ...
    if (!smlua_functions_valid_param_count(L, 0)) {
        return 0;
    }
...
int smlua_func_initiate_warp(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 4)) {
        return 0;
    }

    s16 destLevel = smlua_to_number(L, 1);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("initiate_warp: Failed to convert parameter 1");
        return 0;
    }
    s16 destArea = smlua_to_number(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("initiate_warp: Failed to convert parameter 2");
        return 0;
    }
    s16 destWarpNode = smlua_to_number(L, 3);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("initiate_warp: Failed to convert parameter 3");
        return 0;
    }
    s32 arg3 = smlua_to_number(L, 4);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("initiate_warp: Failed to convert parameter 4");
        return 0;
    }
...
int smlua_func_network_init_object(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 3)) {
        return 0;
    }

    struct Object *obj = smlua_to_cobject(L, 1, LOT_OBJECT);
    if (!gSmLuaConvertSuccess || obj == NULL) {
        LOG_LUA("network_init_object: Failed to convert parameter 1");
        return 0;
    }

    bool standardSync = smlua_to_boolean(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("network_init_object: Failed to convert parameter 2");
        return 0;
    }

    if (lua_type(L, 3) != LUA_TNIL && lua_type(L, 3) != LUA_TTABLE) {
        LOG_LUA_LINE("network_init_object() called with an invalid type for param 3: %u",
                     lua_type(L, 3));
        return 0;
    }
    ...
    if (lua_type(L, 3) == LUA_TTABLE) {
        ...
        while (lua_next(L, 3) != 0) {
            ...
            u8 lvtSize = 0;
            if ((data->valueType == LVT_U32) || (data->valueType == LVT_S32)
                || (data->valueType == LVT_F32)) {
                lvtSize = 32;
            }
            if ((data->valueType == LVT_U16) || (data->valueType == LVT_S16)) {
                lvtSize = 16;
            }
            if ((data->valueType == LVT_U8) || (data->valueType == LVT_S8)) {
                lvtSize = 8;
            }
...
int smlua_func_network_send_object(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 2)) {
        return 0;
    }

    struct Object *obj = smlua_to_cobject(L, 1, LOT_OBJECT);
    if (!gSmLuaConvertSuccess || obj == NULL) {
        LOG_LUA("network_send_object: Failed to convert parameter 1");
        return 0;
    }

    bool reliable = smlua_to_boolean(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("network_send_object: Failed to convert parameter 2");
        return 0;
    }
...
int smlua_func_djui_hud_render_texture(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 5)) {
        return 0;
    }
    ...
    if (smlua_is_cobject(L, 1, LOT_TEXTUREINFO)) {
        ...
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture: Failed to convert parameter 1");
            return 0;
        }
    } else {
        int top = lua_gettop(L);
        ...
        lua_pop(L, 1);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture: Failed to convert parameter 1's 'texture' field");
            return 0;
        }

        tmpTexInfo.bitSize = smlua_get_integer_field(top + 1, "bitSize");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture: Failed to convert parameter 1's 'bitSize' field");
            return 0;
        }

        tmpTexInfo.width = smlua_get_integer_field(top + 1, "width");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture: Failed to convert parameter 1's 'width' field");
            return 0;
        }

        tmpTexInfo.height = smlua_get_integer_field(top + 1, "height");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture: Failed to convert parameter 1's 'height' field");
            return 0;
        }

        tmpTexInfo.name = smlua_get_string_field(top + 1, "name");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture: Failed to convert parameter 1's 'name' field");
            return 0;
        }
    ...
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture: Failed to convert parameter 2");
        return 0;
    }
    f32 y = smlua_to_number(L, 3);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture: Failed to convert parameter 3");
        return 0;
    }
    f32 scaleW = smlua_to_number(L, 4);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture: Failed to convert parameter 4");
        return 0;
    }
    f32 scaleH = smlua_to_number(L, 5);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture: Failed to convert parameter 5");
        return 0;
    }

    djui_hud_render_texture_raw(texInfo->texture, texInfo->bitSize, texInfo->width, texInfo->height, x,
                                y, scaleW, scaleH);
...
int smlua_func_djui_hud_render_texture_tile(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 9)) {
        return 0;
    }
    ...
    if (smlua_is_cobject(L, 1, LOT_TEXTUREINFO)) {
        texInfo = (struct TextureInfo *) smlua_to_cobject(L, 1, LOT_TEXTUREINFO);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 1");
            return 0;
        }
    } else {
        int top = lua_gettop(L);
        ...
        lua_pop(L, 1);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 1's 'texture' field");
            return 0;
        }

        tmpTexInfo.bitSize = smlua_get_integer_field(top + 1, "bitSize");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 1's 'bitSize' field");
            return 0;
        }

        tmpTexInfo.width = smlua_get_integer_field(top + 1, "width");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 1's 'width' field");
            return 0;
        }

        tmpTexInfo.height = smlua_get_integer_field(top + 1, "height");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 1's 'height' field");
            return 0;
        }
    ...
    f32 x = smlua_to_number(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 2");
        return 0;
    }
    f32 y = smlua_to_number(L, 3);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 3");
        return 0;
    }
    f32 scaleW = smlua_to_number(L, 4);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 4");
        return 0;
    }
    f32 scaleH = smlua_to_number(L, 5);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 5");
        return 0;
    }
    f32 tileX = smlua_to_number(L, 6);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 6");
        return 0;
    }
    f32 tileY = smlua_to_number(L, 7);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 7");
        return 0;
    }
    f32 tileW = smlua_to_number(L, 8);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 8");
        return 0;
    }
    f32 tileH = smlua_to_number(L, 9);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile: Failed to convert parameter 9");
        return 0;
    }

    djui_hud_render_texture_tile_raw(texInfo->texture, texInfo->bitSize, texInfo->width,
                                     texInfo->height, x, y, scaleW, scaleH, tileX, tileY, tileW, tileH);
...
int smlua_func_djui_hud_render_texture_interpolated(lua_State *L) {
    ...
    if (smlua_is_cobject(L, 1, LOT_TEXTUREINFO)) {
        texInfo = (struct TextureInfo *) smlua_to_cobject(L, 1, LOT_TEXTUREINFO);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 1");
            return 0;
        }
    } else {
        int top = lua_gettop(L);
        ...
        lua_pop(L, 1);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 1's 'texture' "
                    "field");
            return 0;
        }
        ...
        tmpTexInfo.bitSize = smlua_get_integer_field(top + 1, "bitSize");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 1's 'bitSize' "
                    "field");
            return 0;
        }
        ...
        if (!gSmLuaConvertSuccess) {
            LOG_LUA(
                "djui_hud_render_texture_interpolated: Failed to convert parameter 1's 'width' field");
            return 0;
        }
        ...
        if (!gSmLuaConvertSuccess) {
            LOG_LUA(
                "djui_hud_render_texture_interpolated: Failed to convert parameter 1's 'height' field");
            return 0;
        }
        ...
        if (!gSmLuaConvertSuccess) {
            LOG_LUA(
                "djui_hud_render_texture_interpolated: Failed to convert parameter 1's 'name' field");
            return 0;
        }
    ...
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 2");
        return 0;
    }
    f32 prevY = smlua_to_number(L, 3);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 3");
        return 0;
    }
    f32 prevScaleW = smlua_to_number(L, 4);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 4");
        return 0;
    }
    f32 prevScaleH = smlua_to_number(L, 5);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 5");
        return 0;
    }
    f32 x = smlua_to_number(L, 6);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 6");
        return 0;
    }
    f32 y = smlua_to_number(L, 7);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 7");
        return 0;
    }
    f32 scaleW = smlua_to_number(L, 8);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 8");
        return 0;
    }
    f32 scaleH = smlua_to_number(L, 9);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_interpolated: Failed to convert parameter 9");
        return 0;
    }

    djui_hud_render_texture_interpolated(texInfo, prevX, prevY, prevScaleW, prevScaleH, x, y, scaleW,
                                         scaleH);
...
int smlua_func_djui_hud_render_texture_tile_interpolated(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 13)) {
        return 0;
    }
    ...
    if (smlua_is_cobject(L, 1, LOT_TEXTUREINFO)) {
        texInfo = (struct TextureInfo *) smlua_to_cobject(L, 1, LOT_TEXTUREINFO);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 1");
            return 0;
        }
    } else {
        int top = lua_gettop(L);
        ...
        lua_pop(L, 1);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 1's "
                    "'texture' field");
            return 0;
        }
        ...
        tmpTexInfo.name = smlua_get_string_field(top + 1, "name");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 1's 'name' "
                    "field");
            return 0;
        }
    ...
    f32 prevX = smlua_to_number(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 2");
        return 0;
    }
    f32 prevY = smlua_to_number(L, 3);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 3");
        return 0;
    }
    f32 prevScaleW = smlua_to_number(L, 4);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 4");
        return 0;
    }
    f32 prevScaleH = smlua_to_number(L, 5);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 5");
        return 0;
    }
    f32 x = smlua_to_number(L, 6);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 6");
        return 0;
    }
    f32 y = smlua_to_number(L, 7);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 7");
        return 0;
    }
    f32 scaleW = smlua_to_number(L, 8);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 8");
        return 0;
    }
    f32 scaleH = smlua_to_number(L, 9);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 9");
        return 0;
    }
    f32 tileX = smlua_to_number(L, 10);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 10");
        return 0;
    }
    f32 tileY = smlua_to_number(L, 11);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 11");
        return 0;
    }
    f32 tileW = smlua_to_number(L, 12);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 12");
        return 0;
    }
    f32 tileH = smlua_to_number(L, 13);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("djui_hud_render_texture_tile_interpolated: Failed to convert parameter 13");
        return 0;
    }

    djui_hud_render_texture_tile_interpolated(texInfo, prevX, prevY, prevScaleW, prevScaleH, x, y,
                                              scaleW, scaleH, tileX, tileY, tileW, tileH);
...
int smlua_func_texture_override_set(lua_State *L) {
    ...
    if (smlua_is_cobject(L, 2, LOT_TEXTUREINFO)) {
        ...
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("texture_override_set: Failed to convert parameter 2");
            return 0;
        }
    } else {
        int top = lua_gettop(L);
        ...
        lua_pop(L, 1);
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("texture_override_set: Failed to convert parameter 2's 'texture' field");
            return 0;
        }

        tmpOverrideTexInfo.bitSize = smlua_get_integer_field(top + 1, "bitSize");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("texture_override_set: Failed to convert parameter 2's 'bitSize' field");
            return 0;
        }

        tmpOverrideTexInfo.width = smlua_get_integer_field(top + 1, "width");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("texture_override_set: Failed to convert parameter 2's 'width' field");
            return 0;
        }

        tmpOverrideTexInfo.height = smlua_get_integer_field(top + 1, "height");
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("texture_override_set: Failed to convert parameter 2's 'height' field");
            return 0;
        }
...
int smlua_func_texture_override_reset(lua_State *L) {
    ...
    const char *textureName = smlua_to_string(L, 1);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("texture_override_reset: Failed to convert parameter 1");
        return 0;
    }
...
static u16 *smlua_to_u16_list(lua_State *L, int index, u32 *length) {
    ...
    *length = lua_rawlen(L, index);
    if (!*length) {
        LOG_LUA("smlua_to_u16_list: Table must not be empty");
        return NULL;
    }
...
int smlua_func_smlua_anim_util_register_animation(lua_State *L) {
    if (!smlua_functions_valid_param_count(L, 8)) {
        return 0;
    }

    const char *name = smlua_to_string(L, 1);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'name'");
        return 0;
    }

    s16 flags = smlua_to_integer(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'flags'");
        return 0;
    }
    ...
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'startFrame'");
        return 0;
    }

    s16 loopStart = smlua_to_integer(L, 5);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'loopStart'");
        return 0;
    }

    s16 loopEnd = smlua_to_integer(L, 6);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'loopEnd'");
        return 0;
    }
    ...
    if (!values) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'values'");
        return 0;
    }
    ...
    if (!index) {
        LOG_LUA("smlua_anim_util_register_animation: Failed to convert parameter 'index'");
        free(values);
        return 0;
    }

    smlua_anim_util_register_animation(name, flags, animYTransDivisor, startFrame, loopStart, loopEnd,
                                       values, valuesLength, index, indexLength);
...
int smlua_func_log_to_console(lua_State *L) {
    if (!smlua_functions_valid_param_range(L, 1, 2)) {
        return 0;
    }
    ...
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("log_to_console: Failed to convert parameter 1 for function");
        return 0;
    }
    ...
    if (paramCount >= 2) {
        ...
        if (!gSmLuaConvertSuccess) {
            LOG_LUA("log_to_console: Failed to convert parameter 2 for function");
            return 0;
        }
    }
...
int smlua_func_add_scroll_target(lua_State *L) {
    ...
    if (!smlua_functions_valid_param_range(L, 2, 4)) {
        return 0;
    }
    int paramCount = lua_gettop(L);
    ...
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("add_scroll_target: Failed to convert parameter 1 for function");
        return 0;
    }
    const char *name = smlua_to_string(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("add_scroll_target: Failed to convert parameter 2 for function");
        return 0;
    }
...
int smlua_func_collision_find_surface_on_ray(lua_State *L) {
    if (!smlua_functions_valid_param_range(L, 6, 7)) {
        return 0;
    }
    int paramCount = lua_gettop(L);
    ...
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 1");
        return 0;
    }
    f32 startY = smlua_to_number(L, 2);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 2");
        return 0;
    }
    f32 startZ = smlua_to_number(L, 3);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 3");
        return 0;
    }
    f32 dirX = smlua_to_number(L, 4);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 4");
        return 0;
    }
    f32 dirY = smlua_to_number(L, 5);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 5");
        return 0;
    }
    f32 dirZ = smlua_to_number(L, 6);
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 6");
        return 0;
    }
    f32 precision = paramCount == 7 ? smlua_to_number(L, 7) : 3.0f;
    if (!gSmLuaConvertSuccess) {
        LOG_LUA("collision_find_surface_on_ray: Failed to convert parameter 7");
        return 0;
    }

    smlua_push_object(
        L, LOT_RAYINTERSECTIONINFO,
...
void smlua_bind_functions(void) {
    ...
    smlua_bind_function(L, "djui_hud_render_texture_interpolated",
                        smlua_func_djui_hud_render_texture_interpolated);
    smlua_bind_function(L, "djui_hud_render_texture_tile_interpolated",
                        smlua_func_djui_hud_render_texture_tile_interpolated);
    ...
    smlua_bind_function(L, "smlua_anim_util_register_animation",
                        smlua_func_smlua_anim_util_register_animation);
