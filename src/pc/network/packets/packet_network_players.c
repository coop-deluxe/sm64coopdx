...
static void network_send_to_network_players(u8 sendToLocalIndex) {
    ...
    for (s32 i = 0; i < MAX_PLAYERS; i++) {
        ...
        LOG_DEBUG_VERBOSE("send network player [%d == %d]", gNetworkPlayers[i].globalIndex, npType);
    }

    network_send_to(sendToLocalIndex, &p);
    LOG_DEBUG_VERBOSE("sent list of %d network players to %d", connectedCount, sendToLocalIndex);
}
...
void network_send_network_players_request(void) {
    ...
    LOG_DEBUG_VERBOSE("sending network players request");
}
...
void network_receive_network_players_request(struct Packet* p) {
    ...
    if (localIndex == UNKNOWN_LOCAL_INDEX) {
        LOG_ERROR_VERBOSE("Received network players request from unknown index");
        return;
    }
    network_send_to_network_players(localIndex);

    if (moderator_list_contains(gNetworkSystem->get_id_str(p->localIndex))) {
        LOG_DEBUG_VERBOSE("sending moderator packet to localIndex: %d", p->localIndex);
...
void network_send_network_players(u8 exceptLocalIndex) {
    SOFT_ASSERT(gNetworkType == NT_SERVER);
    LOG_DEBUG_VERBOSE("sending list of network players to all");
...
void network_receive_network_players(struct Packet *p) {
    LOG_DEBUG_VERBOSE("receiving list of network players");
    if (gNetworkType != NT_CLIENT) {
        LOG_ERROR_VERBOSE("received list of clients as a non-client");
        return;
    }
    u8 connectedCount = 0;
    ...
    for (s16 i = 0; i < connectedCount; i++) {
        ...
        LOG_DEBUG_VERBOSE("received network player [%d == %d] (%d)", globalIndex, npType, localIndex);
        if (localIndex != UNKNOWN_GLOBAL_INDEX) {
            ...
            if (localIndex != 0) {
                ...
                LOG_DEBUG_VERBOSE("received network player location (%d, %d, %d, %d)", courseNum, actNum, levelNum, areaIndex);
