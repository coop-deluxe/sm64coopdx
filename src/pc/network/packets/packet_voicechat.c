...
void network_receive_voicechat_frame(struct Packet* p) {
    ...
    voicechat_decode_audio(
        network_local_index_from_global(global_index),
        frame_data, frame_size
    );
}

void network_send_voicechat_muted(u8 receiverIndex, u8 mask, bool muted) {
    struct Packet p = {};
    ...
    if (mask == VOICECHAT_MUTE_LOCAL) network_send_to(network_local_index_from_global(receiverIndex), &p);
    else network_send(&p);
}
