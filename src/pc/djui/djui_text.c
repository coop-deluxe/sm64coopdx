...
struct DjuiText* djui_text_create(struct DjuiBase* parent, const char* message) {
    ...
    text->message = NULL;
    djui_text_set_font(text, gDjuiFonts[configDjuiThemeFont == 1 ? FONT_ALIASED : FONT_ALIASED ? configDjuiThemeFont == 0 ? FONT_NORMAL : FONT_NORMAL ? configDjuiThemeFont == 2 : FONT_EXCOOP : FONT_EXCOOP]);
