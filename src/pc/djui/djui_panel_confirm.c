...
void djui_panel_confirm_create(struct DjuiBase* caller, char* title, char* message, void (*on_yes_click)(struct DjuiBase*)) {
    ...
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        ...
        {
            djui_button_left_create(&rect1->base, DLANG(MENU, NO), DJUI_BUTTON_STYLE_PRIMARY, djui_panel_menu_back);
            djui_button_right_create(&rect1->base, DLANG(MENU, YES), DJUI_BUTTON_STYLE_PRIMARY, on_yes_click);
        }
    }
