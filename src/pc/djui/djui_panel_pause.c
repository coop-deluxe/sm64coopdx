...
void djui_panel_pause_create(struct DjuiBase* caller) {
    ...
    struct DjuiThreePanel* panel = djui_panel_menu_create("PAUSE");
    struct DjuiBase* body = djui_three_panel_get_body(panel);
    {
        ...
        struct DjuiButton* button3 = djui_button_create(body, "Options", DJUI_BUTTON_STYLE_NORMAL, djui_panel_options_create);
        ...
        if (gNetworkType == NT_SERVER) {
            djui_button_create(body, "Server Settings", DJUI_BUTTON_STYLE_NORMAL, djui_panel_host_create);
        }

        djui_button_create(body, "Resume", DJUI_BUTTON_STYLE_NORMAL, djui_panel_pause_resume);

        if (gNetworkType == NT_SERVER) {
            djui_button_create(body, "Stop Hosting", DJUI_BUTTON_STYLE_BACK, djui_panel_pause_quit);
        } else {
            djui_button_create(body, "Disconnect", DJUI_BUTTON_STYLE_BACK, djui_panel_pause_quit);
        }
    }
