...
bool djui_inputbox_on_key_down(struct DjuiBase *base, int scancode) {
    ...
    switch (scancode) {
        case SCANCODE_SHIFT_LEFT:    heldShift   |= (1 << 0); return true;
        case SCANCODE_SHIFT_RIGHT:   heldShift   |= (1 << 1); return true;
        case SCANCODE_CONTROL_LEFT:  heldControl |= (1 << 0); return true;
        case SCANCODE_CONTROL_RIGHT: heldControl |= (1 << 1); return true;
        case SCANCODE_ALT_LEFT:      heldAlt     |= (1 << 0); return true;
        case SCANCODE_ALT_RIGHT:     heldAlt     |= (1 << 1); return true;
    }

    // [Left], [Ctrl]+[Left], [Shift]+[Left], [Ctrl]+[Shift]+[Left]
    if (!heldAlt && scancode == SCANCODE_LEFT) {
        if (heldControl) {
            sel[0] = djui_inputbox_jump_word_left(msg, len, sel[0]);
        } else if (sel[0] > 0) {
            sel[0]--;
        }
        if (!heldShift) { sel[1] = sel[0]; }
        sCursorBlink = 0;
        return true;
    }

    // [Right], [Ctrl]+[Right], [Shift]+[Right], [Ctrl]+[Shift]+[Right]
    if (!heldAlt && scancode == SCANCODE_RIGHT) {
        if (heldControl) {
    ...
    // [Home], [Shift]+[Home]
    if (!heldAlt && scancode == SCANCODE_HOME) {
        sel[0] = 0;
        if (!heldShift) { sel[1] = sel[0]; }
        sCursorBlink = 0;
        return true;
    ...
    // [End], [Shift]+[End]
    if (!heldAlt && scancode == SCANCODE_END) {
        sel[0] = len;
        if (!heldShift) { sel[1] = sel[0]; }
        sCursorBlink = 0;
        return true;
    ...
    if (!heldAlt && scancode == SCANCODE_BACKSPACE) {
        if (sel[0] == sel[1]) {
            if (heldControl) {
                sel[0] = djui_inputbox_jump_word_left(msg, len, sel[0]);
            } else if (sel[0] > 0) {
    ...
    // [Delete], [Ctrl]+[Delete]
    if (!heldAlt && scancode == SCANCODE_DELETE) {
        if (sel[0] == sel[1]) {
            if (heldControl) {
    ...
    // [Ctrl]+[V], [Shift]+[Insert]
    if (!heldAlt &&
        ((!heldShift && heldControl && scancode == SCANCODE_V) ||
        (!heldControl && heldShift && scancode == SCANCODE_INSERT))) {
    ...
    // [Ctrl]+[C], [Ctrl]+[X]
    if (!heldAlt && !heldShift && heldControl &&
    ...
    // [Ctrl]+[A]
    if (!heldAlt && !heldShift && heldControl && scancode == SCANCODE_A) {
    ...
    // [Esc]
    if (!heldAlt && !heldShift && !heldControl && scancode == SCANCODE_ESCAPE) {
    ...
    // [Enter]
    if (!heldAlt && !heldShift && !heldControl && scancode == SCANCODE_ENTER) {
        djui_interactable_set_input_focus(NULL);
...
void djui_inputbox_on_key_up(UNUSED struct DjuiBase *base, int scancode) {
    switch (scancode) {
        case SCANCODE_SHIFT_LEFT:    heldShift   &= ~(1 << 0); break;
        case SCANCODE_SHIFT_RIGHT:   heldShift   &= ~(1 << 1); break;
        case SCANCODE_CONTROL_LEFT:  heldControl &= ~(1 << 0); break;
        case SCANCODE_CONTROL_RIGHT: heldControl &= ~(1 << 1); break;
        case SCANCODE_ALT_LEFT:      heldAlt     &= ~(1 << 0); break;
        case SCANCODE_ALT_RIGHT:     heldAlt     &= ~(1 << 1); break;
    }
}
