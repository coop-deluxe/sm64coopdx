...
u32 attack_object(struct MarioState* m, struct Object *o, s32 interaction) {
    ...
    smlua_call_event_hooks_HOOK_ON_ATTACK_OBJECT(m, o, interaction);
    return attackType;
}
...
#define PLAYER_IN_ROLLOUT_FLIP(m) ((m->action == ACT_FORWARD_ROLLOUT || m->action == ACT_BACKWARD_ROLLOUT) && m->actionState == 1)
    ...
    // call the Lua hook
    bool allow = true;
    smlua_call_event_hooks_HOOK_ALLOW_PVP_ATTACK(attacker, cVictim, interaction, &allow);
    if (!allow) {
...
#undef PLAYER_IN_ROLLOUT_FLIP
    ...
    smlua_call_event_hooks_HOOK_ON_PVP_ATTACK(attacker, victim, interaction);
    return FALSE;
}
...
u32 process_interaction(struct MarioState *m, u32 interactType, struct Object *o, u32 (*interact_function)(struct MarioState *, u32 interactType, struct Object *)) {
    if (!m || !o) { return FALSE; }
    bool allow = true;
    smlua_call_event_hooks_HOOK_ALLOW_INTERACT(m, o, interactType, &allow);
    if (allow) {
        if (interact_function(m, interactType, o)) {
            smlua_call_event_hooks_HOOK_ON_INTERACT(m, o, interactType, true);
            return TRUE;
        } else {
            smlua_call_event_hooks_HOOK_ON_INTERACT(m, o, interactType, false);
        }
    }
    return FALSE;
}
...
void mario_process_interactions(struct MarioState *m) {
        ...
        for (i = 0; i < 32; i++) {
            ...
            if (m->collidedObjInteractTypes & interactType) {
                ...
                if (object && !(object->oInteractStatus & INT_STATUS_INTERACTED)) {
                    bool allow = true;
                    smlua_call_event_hooks_HOOK_ALLOW_INTERACT(m, object, interactType, &allow);
                    if (allow) {
                        if (sInteractionHandlers[i].handler(m, interactType, object)) {
                            smlua_call_event_hooks_HOOK_ON_INTERACT(m, object, interactType, true);
                            break;
                        } else {
                            smlua_call_event_hooks_HOOK_ON_INTERACT(m, object, interactType, false);
                        }
                    }
                }
            }
    ...
    smlua_call_event_hooks_HOOK_ON_INTERACTIONS(m);
...
void check_lava_boost(struct MarioState *m) {
    if (!m) { return; }
    bool allow = true;
    smlua_call_event_hooks_HOOK_ALLOW_HAZARD_SURFACE(m, HAZARD_TYPE_LAVA_FLOOR, &allow);
