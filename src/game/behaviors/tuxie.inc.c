...
void tuxies_mother_act_0(void) {
    // only local can interact with mother
    struct MarioState* marioState = &gMarioStates[0];
    s32 distanceToPlayer = marioState->visibleToObjects == 1 ? dist_between_objects(o, marioState->marioObj) : 10000;
    ...
    if (sp24 != NULL && sp24->heldByPlayerIndex == 0 && sp28 < 300.0f && sp24->oHeldState != HELD_FREE) {
        ...
        switch (o->oSubAction) {
            case 0:
                ...
                break;
            case 1:
                if (marioState->visibleToObjects == 1 && cur_obj_update_dialog_with_cutscene(marioState, 2, 1, CUTSCENE_DIALOG, gBehaviorValues.dialogs.TuxieMotherDialog, tuxies_mother_act_0_continue_dialog))
                    o->oSubAction++;
                break;
            case 2:
