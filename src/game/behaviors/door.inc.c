...
struct DoorAction sDoorActions[] = { { 0x40000, 3 }, { 0x80000, 4 }, { 0x10000, 1 }, { 0x20000, 2 }, { -1, 0 }, };

s32 sOpenDoorSounds[] = { SOUND_GENERAL_OPEN_WOOD_DOOR, SOUND_GENERAL_OPEN_IRON_DOOR };

s32 sCloseDoorSounds[] = { SOUND_GENERAL_CLOSE_WOOD_DOOR, SOUND_GENERAL_CLOSE_IRON_DOOR };
...
void play_door_open_noise(void) {
    s32 doorType = cur_obj_has_model(MODEL_HMC_METAL_DOOR);
    if (o->oTimer == 0) {
        cur_obj_play_sound_and_rumble_if_visible(sOpenDoorSounds[doorType]);
        //gTimeStopState |= TIME_STOP_MARIO_OPENED_DOOR;
    }
    if (o->oTimer == 70) {
        cur_obj_play_sound_and_rumble_if_visible(sCloseDoorSounds[doorType]);
    }
}
...
void bhv_door_loop(void) {
    ...
    if (o->oAction != 100) {
        while (sDoorActions[actionIndex].flag != (u32)~0) {
            if (cur_obj_clear_interact_status_flag(sDoorActions[actionIndex].flag)) {
                ...
                cur_obj_change_action(sDoorActions[actionIndex].action);
            }
            actionIndex++;
        }
    }
