...
void hexagonal_ring_spawn_flames(void) {
    struct Object *volcanoFlamesObj = spawn_object(o, MODEL_RED_FLAME, bhvVolcanoFlames);
    if (volcanoFlamesObj != NULL) {
        volcanoFlamesObj->oPosY += 550.0f;
        volcanoFlamesObj->oMoveAngleYaw = random_u16() << 0x10 >> 0x10;
        volcanoFlamesObj->oForwardVel = random_float() * 40.0f + 20.0f;
        volcanoFlamesObj->oVelY = random_float() * 50.0f + 10.0f;
        f32 size = random_float() * 6.0 + 3.0;
        obj_scale_xyz(volcanoFlamesObj, size, size, size);
    }
