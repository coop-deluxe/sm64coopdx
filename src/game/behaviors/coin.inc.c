...
void bhv_coin_loop(void) {
    struct Surface *floorSurface;
    cur_obj_update_floor_and_walls();
    cur_obj_if_hit_wall_bounce_away();
    cur_obj_move_standard(-62);
    if ((floorSurface = o->oFloor) != NULL) {
        ...
        if (o->oSubAction == 1) {
            o->oBounciness = 0;
            if (floorSurface->normal.y < 0.9) {
                s16 targetYaw = atan2s(floorSurface->normal.z, floorSurface->normal.x);
...
void spawn_coin_in_formation(s32 index, s32 shape) {
    ...
    switch (shape & 7) {
        ...
        case 1:
            snapToGround = 0;
            coinPos[1] = 160 * index * 0.8; // 128 * index
            if (index > 4) {
                spawnCoin = 0;
            }
            break;
        case 2:
            coinPos[0] = sins(index << 13) * 300.0f;
            coinPos[2] = coss(index << 13) * 300.0f;
            break;
        case 3:
            snapToGround = 0;
            coinPos[0] = coss(index << 13) * 200.0f;
            coinPos[1] = sins(index << 13) * 200.0f + 200.0f;
            break;
        case 4:
            coinPos[0] = sCoinArrowPositions[index][0];
            coinPos[2] = sCoinArrowPositions[index][1];
            break;
    }
    if (shape & 0x10) {
        snapToGround = 0;
    ...
    if (spawnCoin) {
        struct Object *yellowCoin = spawn_object_relative(index, coinPos[0], coinPos[1], coinPos[2], o, MODEL_YELLOW_COIN, bhvCoinFormationSpawn);
        if (yellowCoin != NULL) { yellowCoin->oCoinUnkF8 = snapToGround; }
    }
}
...
void coin_inside_boo_act_0(void) {
    ...
    if (parent->oBooDeathStatus == BOO_DEATH_STATUS_DYING) {
        o->oAction = 1;
        s16 yawAngle = gMarioObject ? gMarioObject->oMoveAngleYaw : 0;
        f32 speed = 3.0f;
        o->oVelX = sins(yawAngle) * speed;
        o->oVelZ = coss(yawAngle) * speed;
        o->oVelY = 35.0f;
        o->parentObj = NULL;
    }
}
...
void bhv_golden_coin_sparkles_loop(void) {
    f32 offsetMultiplier = 30.0f;
    struct Object *sparkles = spawn_object(o, MODEL_SPARKLES, bhvCoinSparkles);
    if (sparkles == NULL) { return; }
    sparkles->oPosX += random_float() * offsetMultiplier - offsetMultiplier / 2;
    sparkles->oPosZ += random_float() * offsetMultiplier - offsetMultiplier / 2;
}
