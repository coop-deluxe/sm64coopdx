...
f32 floating_platform_find_home_y(void) {
    ...
    floorHeight = find_floor(o->oPosX, o->oPosY, o->oPosZ, &surface);
...
void floating_platform_act_0(void) {
    ...
    if (playersTouched > 0) {
        ...
        s16 localX = (x - o->oPosX) * coss(-1*o->oMoveAngleYaw) + (z - o->oPosZ) * sins(-1*o->oMoveAngleYaw);
        s16 localZ = (z - o->oPosZ) * coss(-1*o->oMoveAngleYaw) - (x - o->oPosX) * sins(-1*o->oMoveAngleYaw);

        o->oFaceAnglePitch = localX * 2;
        o->oFaceAngleRoll = -localZ * 2;
        o->oVelY -= 1.0f;
        if (o->oVelY < 0.0f) {
            o->oVelY = 0.0f;
        }
