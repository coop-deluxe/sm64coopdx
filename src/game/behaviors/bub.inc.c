...
void bub_spawner_act_0(void) {
    s32 amount = o->oBirdChirpChirpUnkF4;
...
void bub_move_vertically(s32 speed) {
    f32 parentY = o->parentObj->oPosY;
    if (parentY - 100.0f - o->oCheepCheepUnk104 < o->oPosY && o->oPosY < parentY + 1000.0f + o->oCheepCheepUnk104) {
        o->oPosY = approach_f32_symmetric(o->oPosY, o->oCheepCheepUnkF8, speed);
    }
}
...
void bub_act_1(void) {
    ...
    if (distanceToPlayer < 200.0f && distanceToPlayer < 0.5) {
        o->oAction = 2;
    }
...
void bub_act_2(void) {
    ...
    s32 angleToPlayer = obj_angle_to_object(o, player);
    f32 dy;
    if (o->oTimer < 20 && o->oInteractStatus & INT_STATUS_INTERACTED) {
