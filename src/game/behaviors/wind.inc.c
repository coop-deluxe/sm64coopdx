...
void bhv_wind_loop(void) {
    s16 posShiftMultiplier = 500;
    f32 windObjScale = 1.0f;
    if (o->oTimer == 0) {
        o->oOpacity = 100;
        if (o->oMoveAnglePitch == 0) {
            obj_translate_xz_random(o, 900.0f);
            o->oPosX += sins(o->oMoveAngleYaw + 0x8000) * posShiftMultiplier; // NOP as Pitch is 0
            o->oPosY += 80.0f + random_f32_around_zero(200.0f);
            o->oPosZ += coss(o->oMoveAngleYaw + 0x8000) * posShiftMultiplier; // -coss(a) * posShiftMultiplier
            o->oMoveAngleYaw += random_f32_around_zero(4000.0f);
            o->oForwardVel = random_float() * 70.0f + 50.0f;
        } else {
            obj_translate_xz_random(o, 600.0f);
            o->oPosY -= posShiftMultiplier - 200; // 300
            o->oVelY = random_float() * 30.0f + 50.0f;
            o->oMoveAngleYaw = random_u16();
            o->oForwardVel = 10.0f;
        }
        obj_set_billboard(o);
        cur_obj_scale(windObjScale);
    }
    if (o->oTimer > 8)
